...
const UserBookingsPage: React.FC = () => {
  const { isAuthenticated, user } = useSelector((state: RootState) => state.authSlice) as { isAuthenticated: boolean; user: any };
  ...
  // Extract bookings from API response and combine with payments
  useEffect(() => {
    if (apiResponse) {
      ...
      // Enhance bookings with payments data
  ...
  useEffect(() => {
    if (selectedBooking) {
      ...
      setPaymentForm({
        phone_number: selectedBooking.phone || user?.phone || '',
